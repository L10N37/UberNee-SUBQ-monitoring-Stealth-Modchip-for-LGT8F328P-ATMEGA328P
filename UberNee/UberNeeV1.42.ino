// 1.42: Add additional support for ATMEGA328P MCU (i.e. nano)
// Timing not tested for using ATMEGA328P + PSONE PAL BIOS patch, *should* be ok.
...
#define pullBIOS bitClear(PORTB, 3), bitSet(DDRB, 3)
#define releaseBIOS bitClear(DDRB, 3)
#define addr18LOW (bitRead(PIND, 2) == LOW)
...
bool PSONE_BIOS_NOT_PATCHED = true;
bool SYSTEMOFF = true;
...
const bool PM41PATCH = no       //<---------------------------------------------> PAL PM41 (PSOne) BIOS patch required?
const bool DEBUGGINGPM41 = no   //<---------------------------------------------> Debugging on a PAL PM41? (which requires BIOS patch)

const bool LGT8F328P = yes      //<---------------------------------------------> If you're using an ATMEGA328P, set this to NO (different code for WFCK frequency for PU22+)
